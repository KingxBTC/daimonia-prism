...
**验收方法**：
1. 对 AC-5（stripe.com）手动跑 §5.A Light 流程
2. 与 Prism 输出对比各维度分
...
> **状态**：待 T4（Analyzer）实现后，由质龙对 stripe.com 跑端到端对比并填入表格。
> 当前 T4 仍 in_progress（[DAI-1274](/DAI/issues/DAI-1274)）。
...
| 任务 | 状态 | 备注 |
|------|------|------|
| §10 错误处理实现 | ✅ 完成 | `src/core/errors.ts` 实现，26 个单元测试全 pass |
| §12.1 验收用例集 | ✅ 规范完成 | 5 站定义齐全；端到端跑通待 T4 Analyzer 完成 |
| §12.2 方法论一致性 | ⏳ 待 T4 | 对比表模板就绪，待实际跑分后填入 |
| §12.3 架构验收 | ✅ 完成 | 渲染层测试 9 项全 pass，架构边界已验 |
| 总测试通过数 | ✅ 112/112 | 含 schema、error、china、collect、render |
